...
st.write("Choirinnisa' Fitria - 200411100149")
...
with upload_data:
    st.write("Dataset ini didapat dari kaggle.com,Di SayoPillow.csv Anda akan melihat hubungan antara parameter - kisaran mendengkur pengguna, laju pernapasan, suhu tubuh, laju pergerakan tungkai, kadar oksigen darah, pergerakan mata, jumlah jam tidur, detak jantung, dan Tingkat Stres (0 - rendah/normal, 1 – sedang rendah, 2-sedang, 3-sedang tinggi, 4-tinggi) aplikasi ini digunakan untuk mengetahui tingkat stres melalui cara tidur seseorang ")
    st.write("Berikut link dataset yang digunakan https://www.kaggle.com/datasets/laavanya/human-stress-detection-in-and-through-sleep?resource=download&select=SaYoPillow.csv")
...
with preporcessing:
    ...
    df[["snoring rate", "respiration rate", "body temperature", "limb movement", "blood oxygen", "eye movement", "sleeping hours", "heart rate", "stress level"]].agg(['min','max'])

    df.stres.value_counts()
    ...
    labels = pd.get_dummies(df.stres).columns.values.tolist()
    ...
    dataubah=df.drop(columns=['respiration rate','limb movement'])
    dataubah

    "### Normalize data respiration rate"
    data_respiration=df[['respiration']]
    respirasi = pd.get_dummies(data_respiration)
    respirasi
    ...
    "### Normalize data limb movement"
    data_limb_movement =df[[' limb movement']]
    limb_movement = pd.get_dummies(data_limb_movement)
    limb_movement


    dataOlah = pd.concat([respirasi,limb_movement], axis=1)
    dataHasil = pd.concat([df,dataOlah], axis = 1)

    X = dataHasil.drop(columns=["respiration rate","limb movement"])
    y = dataHasil.stres
    ...
    labels = pd.get_dummies(dataHasil.stres).columns.values.tolist()
